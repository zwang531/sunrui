...
def index(request):
    ...
    project = GetDetail(5320)
    starttime = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(project.starttime))
    endtime = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(project.endtime))
    return render(request, 'sunrui/index.html', {'project': project, 'starttime': starttime, 'endtime': endtime})
